...
アプリケーションの起動には `Node.js` ([.tool-versions](./.tool-versions)) , `yarn`, `docker-compose` が必要になる。  
テストおよびビルドまで完全に動作させる場合は、上記に加え `k6`, `tbls`, `redoc-cli`, `swagger-codegen` を導入した環境を構築する。
...
| サービスの停止               | `yarn stop`           | ローカル環境で起動中のサービスを停止する                                                                  |
...
- `✔︎`: 必要 (`GitHub > Secrets > Actions` に登録する)
- `-`: 不要 (環境変数ファイルに定義済み)
- `(空)`: 不要
