  ...
  test('プロダクトが存在する場合、コンポーネントが表示されること', () => {
    ...
    dummy_products
      ...
      .map((product, i) => {
        ...
        if (0 < product.images.length)
          expect(screen.getAllByTestId(`Products__Product${i}__Image0`).length).not.toBe(0);
        else expect(screen.queryAllByTestId(`Products__Product${i}__Image0`).length).toBe(0);
      });
    ...
    dummy_products
      ...
      .map((product, index) => {
        ...
        expect(screen.queryAllByTestId(`Products__Product${i}__Image0`).length).toBe(0);
      });
  });
