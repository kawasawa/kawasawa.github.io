...
import { extractAlt } from '@/utils/images';
            ...
            <Grid container spacing={4} sx={{ mt: 2 }}>
              {products
                          ...
                          <Grid item xs={12} sm={7} order={{ xs: i, sm: i % 2 === 0 ? i : i + 1 }}>
                            ...
                            <ChipList sx={{ my: 1, justifyContent: 'center' }}>
                              {product.skills.map((skill, j) => (
                                ...
                                >
                                  <img src={skill.icon} alt={extractAlt(skill.icon)} loading="lazy" decoding="async" />
                                </ChipListItem>
                              ))}
                            </ChipList>
                            <Typography
                ...
                : [...Array(values.skeltonCount.products).keys()].map((i) => (
                    <Box key={`Products__Product${i}--Loading`} data-testid={`Products__Product${i}--Loading`}>
                      <Grid item xs={12} sm={7} order={{ xs: i, sm: i % 2 === 0 ? i : i + 1 }}>
                ...
                <AccordionDetails sx={{ p: 0 }}>
                  {products
                    ...
                    .map((product, index) => {
                      ...
                      const subject = product[`subject_${i18n.language}` as keyof typeof product] as string;
                      const body = product[`body_${i18n.language}` as keyof typeof product] as string;
                      const i = index + (products?.filter((product) => product.visible && product.pickup).length ?? 0);
                      return (
