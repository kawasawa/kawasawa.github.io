...
describe('Articles', () => {
  ...
  test('記事が存在する場合、コンポーネントが表示されること', () => {
  ...
  test('記事が存在しない場合、スケルトンスクリーンが表示されること', () => {
    render(<Articles />);

    expect(screen.getByTestId('Articles__SectionTitle')).toBeVisible();
    [...Array(values.ARTICLES_SKELETON_DISPLAY_COUNT).keys()].map(async (i) => {
      await waitFor(() => expect(screen.getByTestId(`Articles__Card${i}__SkeletonBody0`)).toBeVisible());
      await waitFor(() => expect(screen.getByTestId(`Articles__Card${i}__SkeletonBody1`)).toBeVisible());
      await waitFor(() => expect(screen.getByTestId(`Articles__Card${i}__SkeletonBody2`)).toBeVisible());
      await waitFor(() => expect(screen.getByTestId(`Articles__Card${i}__SkeletonBody3`)).toBeVisible());
      await waitFor(() => expect(screen.getByTestId(`Articles__Card${i}__SkeletonTag0`)).toBeVisible());
      await waitFor(() => expect(screen.getByTestId(`Articles__Card${i}__SkeletonTag1`)).toBeVisible());
    });
  });
