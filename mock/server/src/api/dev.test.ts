...
import { CSRF_COOKIE_NAME } from '../app';
...
describe('dev', () => {
  ...
  //     expect(data).toEqual({ success: true, value: 'dummy_csrf_token' });
  //   });
  // });
  describe('csrfToken', () => {
    test('CSRF トークンが返却されること', () => {
      ...
      expect(res.cookies[CSRF_COOKIE_NAME].value).toBe('dummy_csrf_token');
    });
  });
  ...
  describe('send', () => {
    test('リクエストボディと同値のオブジェクトが返却されること', async () => {
      ...
      const data = res._getJSONData();
      expect(res.statusCode).toBe(200);
      expect(data).toEqual({ success: true, value: { key: 'value' } });
    });
  });
});
