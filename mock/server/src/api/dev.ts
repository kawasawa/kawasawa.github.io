...
export const error = observeError(() => {
  throw new Error('dummy error');
});

export const boomError = observeError(() => {
  throw boom.internal('dummy boom error');
});

export const asyncError = observeError(async () => {
  throw new Error('dummy async error');
});

export const csrfToken = observeError((req: Request, res: Response) => {
...
export const send = observeError(async (req: Request, res: Response) => {
  const params = await createSchema().validate(req.body);
  info(req, 'mail send', params);
  res.status(200).json({ success: true, value: params });
});
