...
const corsOptions: cors.CorsOptions = {
  ...
  //   (登録系メソッドは、リソース全般の新規登録や送信を POST, レコード等の情報の一部更新を PATCH, ファイル等の置換になる更新を PUT として想定する)
...
export const createApp = () => {
  ...
  // Content-Typeフィルタ
  //   CORS 仕様外の `Content-Type: application/json` が指定されることで (POST, PATCH, PUT, DELETE 時に) プリフライトリクエストが発生する
  //   プリフライトリクエストを受けたサーバは CORS ポリシー (Origin や Allow-Method) を確認し、後続リクエストを許可するかどうかを決定する
  ...
  app.use((err: any, req: Request, res: Response, next: NextFunction) => {
    ...
    const body = { error: { code: b.output.statusCode, message: `${b.output.payload.error}: ${b.message}` } };
