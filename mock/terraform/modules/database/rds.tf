...
# 2. 自動バックアップ + トランザクションログ
#    backup_window の設定により日次でバックアップを実行され、加えて 5 分置きにトランザクションログも退避している
#    復元の際はまずバックアップファイルを DB に戻し (リストア)、これに対してトランザクションログを適用 (リカバリ) していく
#    AWS のトランザクションログの退避間隔の制約で 5 分以内のデータ損失は避けられない
#    また、復元の際は新しい DB が作成されるため、既存 DB にレコードを移植するか、アプリ側の向き先設定を変更する必要がある
...
resource "terraform_data" "main" {
  ...
  provisioner "local-exec" {
    command = <<-EOT
      aws logs put-retention-policy \
        --log-group-name "/aws/rds/instance/${aws_db_instance.main.identifier}/slowquery" \
