...
# ECR は AWS のグローバルリソースであるため、プライベートサブネットにある ECS がイメージを pull する際は NAT GW 経由で外部に通信を行う
# これは VPC 外ではあるがインターネットに出ているわけではなく AWS 内のグローバルな領域を通るだけとのこと (AWS の見解では)
