...
#
# ちなみに権威 DNS サーバの前段に jp や com を振り分けるルートサーバが存在し、
# 以降ドメインの階層に沿って複数の権威 DNS サーバがあり、トップレベルドメインから順に名前解決を行う
#   1. ルートサーバ: jp の権威 DNS サーバの情報を返却
#   2. トップレベルドメインサーバ: co.jp の権威 DNS サーバの情報を返却
#   3. ドメインサーバ: example.co.jp の権威 DNS サーバの情報を返却
#   4. ドメインサーバ: example.co.jp の IP アドレスを返却
#
